...
function loopalo(bundle, config, resolver, handler, callback) {
	...
	try {
		configBundle.forEach(function processBundle(b) {
			...
			var resolved = resolver.resolve(path.normalize(b), config.locality);
			if (!resolved.file) {
				var err = new Error("ENOENT: no such file or directory '" + path.normalize(b) + "'");
				err.code = 'ENOENT';
				err.path = path.normalize(b);
				throw err;
			} else {
				bundleRenderer[bundleKey[b]] = function (cb) {
					handler(resolved.file, cb);
				};
			}
		});
	} catch (e) {
